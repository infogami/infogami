...
def _property(name, expected_type, unique=True, description='', **kw):
    return dict(
        ...
        unique={'type': '/type/boolean', 'value': unique},
...
def primitive_types():
    ...
    return [
        f(
            '/type/key',
            'Key',
            'Type to store keys. A key is a string constrained to the regular expression [a-z][a-z/_]*.',
        ),
        f(
            '/type/string',
            'String',
            'Type to store unicode strings up to a maximum length of 2048.',
        ),
        f(
            '/type/text',
            'Text',
            'Type to store arbitrary long unicode text. Values of this type are not indexed.',
        ),
        f(
            '/type/int',
            'Integer',
            'Type to store 32-bit integers. This can store integers in the range [-2**32, 2**31-1].',
        ),
        f('/type/boolean', 'Boolean', 'Type to store boolean values true and false.'),
        f(
            '/type/float',
            'Floating Point Number',
            'Type to store 32-bit floating point numbers',
        ),
        f(
            '/type/datetime',
            'Datetime',
            'Type to store datetimes from 4713 BC to 5874897 AD with 1 millisecond resolution.',
        ),
    ]
...
def system_types():
    return [
        ...
        _type(
            '/type/object', 'Object', 'placeholder type for storing arbitrary objects'
        ),
        _type(
            '/type/dict',
            'Dict',
            'placeholder type for storing arbitrary dictionaties',
            kind='embeddable',
        ),
